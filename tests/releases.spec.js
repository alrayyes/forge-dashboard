...
test.describe('release history page', () => {
  test('reachable without a session, unlike the dashboard itself', async ({ page }) => {
  ...
  test('loads real release data from GitHub, or falls back to a visible link rather than breaking', async ({ page }) => {
    await page.goto('/releases.html');
    ...
    if (releaseCount > 0) {
      ...
      await expect(first.locator('h2 a')).toHaveAttribute('href', /github\.com/);
      await expect(first.locator('.release-date')).not.toHaveText('');
    } else {
      await expect(page.locator('#status')).toContainText(/could not load/i);
      await expect(page.locator('#status a')).toHaveAttribute('href', /github\.com\/alrayyes\/forge-dashboard\/releases/);
    }
  });

  test('the footer links to release history from other pages, but not from itself', async ({ page }) => {
    await registerAndSignIn(page);

    const historyLink = page.locator('#footer-version a', { hasText: 'Release history' });
    await expect(historyLink).toBeVisible();
    await historyLink.click();
    await expect(page).toHaveURL(/\/releases\.html$/);

    await expect(page.locator('#footer-version')).not.toContainText('Release history');
  });
